...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      ...
      if (currentSession) {
        await Promise.all([fetchContests(), fetchUserAndWallet(currentSession.user.id, currentSession)]);
        syncPending();
      } else {
        fetchContests();
      }
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col font-sans selection:bg-red-500/30">
      <nav className="bg-slate-800/60 backdrop-blur-xl border-b border-white/5 p-4 sticky top-0 z-50">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <button onClick={() => { setView('home'); setGameState('ready'); setIsMobileMenuOpen(false); }} className="active:scale-95 transition-transform">
